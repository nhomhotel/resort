...
class IntroSlider extends AdminHome {
    ...
    function index() {
        ...
        $total = $this->Intro_Slider_model->get_total();

        $config = array();
        $config["total_rows"] = $total;
        $config['base_url'] = base_url('admin/IntroSlider/index');
        ...
        $input['order'] = array('intro_slider_id', 'ASC');

        $list = $this->Intro_Slider_model->get_list($input);
        $data['total'] = $total;
        $data['list'] = $list;
        $data['start'] = $start;

        $data['title'] = 'Danh sách Slider';
        $data['temp'] = 'admin/intro_slider/index';
    ...
    function create() {
        ...
        if ($this->input->post()) {
            ...
            if (empty($_FILES['image_intro_slider']['name']))
                $this->form_validation->set_rules('image_intro_slider', 'Ảnh', 'required');
            if ($this->form_validation->run()) {
                $link_intro_slider = $this->input->post('link_intro_slider') ? $this->input->post('link_intro_slider') : '';
                $view_home = $this->input->post('view_home') ? $this->input->post('view_home') : 0;
                $upload_path = './uploads/homeslider';
                $this->load->library('upload_library');
                $image_data = $this->upload_library->upload($upload_path, 'image_intro_slider');
                ...
                $data = array(
                    'image' => $image,
                    'view_home' => $view_home,
                    'link' => $link_intro_slider,
                );
                if ($this->Intro_Slider_model->create($data)) {
                    $this->session->set_flashdata('message', 'Thêm dữ liệu thành công!');
                } else {
                    $this->session->set_flashdata('message', 'Thêm dữ liệu thất bại!');
                }
                redirect(base_url('admin/IntroSlider'));
            }
        }

        $data['title'] = 'Thêm mới Slider';
        $data['temp'] = 'admin/intro_slider/create';
        $this->load->view('admin/layout', isset($data) ? ($data) : NULL);
    }
    ...
    function edit($id) {
        ...
        $this->load->helper('form');
        if ($id > 0) {
            $info = $this->Intro_Slider_model->get_info($id);
            if (!$info) {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run()) {
                    ...
                    if ($this->Intro_Slider_model->update($id, $data)) {
            ...
            $data['temp'] = 'admin/intro_slider/edit';
            $this->load->view('admin/layout', isset($data) ? ($data) : NULL);
        }
    }
    ...
    function delete($id) {
        if ($id > 0) {
            $data['info'] = $this->Intro_Slider_model->get_info($id);
            if (!$data['info']) {
                $this->session->set_flashdata('message', 'Không tồn tại bản ghi!');
                redirect(base_url('admin/IntroSlider'));
            }
            if ($this->Intro_Slider_model->delete($id))
                $this->session->set_flashdata('message', 'Xóa dữ liệu thành công!');
        }
        redirect(base_url('admin/intro_slider'));
    }
    ...
    function deleteAll() {
        if ($this->input->post('arrId')) {
            ...
            foreach ($arrId as $id) {
                $this->Intro_Slider_model->delete($id);
            }
        }
    }
    ...
    function status() {
        ...
        $statusInfo = $this->Intro_Slider_model->get_info($id, 'view_home');
        if (!$statusInfo) {

            $this->session->set_flashdata('message', 'Không tồn tại bản ghi!');
            redirect(admin_url('IntroSlider/index'));
        } else {
